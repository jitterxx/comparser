...
class PredictorNew(object):
    desc = CPO.CLIENT_NAME
    service_url = ''

    def __init__(self):
        if not CPO.PREDICT_SERVICE_HOSTNAME or not CPO.PREDICT_SERVICE_PORT:
            self.service_url = '{}detect'.format(CPO.main_link)
        else:
            self.service_url = 'http://{}:{}/api/detect'.format(CPO.PREDICT_SERVICE_HOSTNAME, CPO.PREDICT_SERVICE_PORT)
    ...
    def create(self, client_name='', service_name=''):
        ...
        try:
            result = json.loads(requests.post('{}/create'.format(self.service_url), data={'service': sname}).text)
        except Exception as e:
            logging.error("Ошибка обращения к серверу API. {}".format(str(e)))
            # Нужно делать уведомления на почту разработчика о таких ошибках
            exit()
        else:
            logging.debug(result)
            if result['status'] != 200:
                logging.error("Ошибка создания сервиса {}_{}. Ошибка: {}".format(client_name, service_name,
                                                                                 result['message']))
                e = Exception()
                e.message = "Ошибка создания сервиса {}_{}. Ошибка: {}".format(client_name, service_name,
                                                                               result['message'])
                raise e
            else:
                logging.debug("*** Сервис создан ***")
    ...
    def check(self, sname=''):
        logging.debug("*** Проверка сервиса {} ***".format(sname))
        logging.debug('{}/info'.format(self.service_url))
        try:
            result = json.loads(requests.get('{}/info'.format(self.service_url)).text)
        except Exception as e:
            logging.error("Ошибка обращения к серверу API. {}".format(str(e)))
            # Нужно делать уведомления на почту разработчика о таких ошибках
            exit()
    ...
    def classify(self, data=None, sname=None):
        ...
        try:
            new_data = dict()
            new_data['message_text'] = data.message_text
            new_data['message_title'] = data.message_title
            new_data['category'] = data.category
            new_data['in_reply_to'] = data.in_reply_to
            new_data['references'] = data.references
            new_data['recipients'] = data.recipients
            new_data['cc_recipients'] = data.cc_recipients
            new_data = json.dumps(new_data)

            result = json.loads(requests.post('{}/predict'.format(self.service_url), data={'service': sname,
                                                                                           'data': new_data}).text)
        except Exception as e:
            logging.error("Ошибка обращения к серверу API. {}".format(str(e)))
            # Нужно делать уведомления на почту разработчика о таких ошибках
...
try:
    ...
    for row in clear:
        ...
        try:
            ...
            short_answer = None
            status = None
            # готовим данные из row
            data = row  # row.message_title + row.message_text
            for service_name in CPO.PREDICT_SERVICE_NAME:
